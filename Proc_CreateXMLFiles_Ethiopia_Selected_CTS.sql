CREATE PROCEDURE [dbo].Proc_CreateXMLFiles_Ethiopia_Selected_CTS (  
...
--------Temporary to be allowed approval by NBE in DD's ------------------ NOT FOR LIVE ENVIRONMENT  
--UPDATE t_DirectDebitMaintenance  
--SET EffectiveDate = @WorkingDate, FirstExecutionDate = @WorkingDate, NextExecutionDate = @WorkingDate, ReturnCode = '00', DDTypeID = 'DDT',   
-- LastExecutionDate = IsNull(LastExecutionDate, @WorkingDate)  
--WHERE DDID NOT IN (  
--  SELECT VATSerialNo  
--  FROM t_TrxClearing(NOLOCK)  
--  WHERE VoucherCode = '40'  
--   AND ReturnCodeID = '00'  
--   AND TrxType = 'OC'  
--  )  
...
--EXEC ps_DirectDebit @OurBranchID = '3401', @ProcessDate = '21 Jun 2023', @ProcessID = 'SIT', @OperatorID = 'Kamunya', @ErrorNo = @ErrorNo OUTPUT  
...
CREATE TABLE #MICREFTs (  
 ...
 , DCNm VARCHAR(100), CCNm VARCHAR(100), DbtrAcct VARCHAR(50), CdtrAcct VARCHAR(50), OrgnlMsgId VARCHAR(100), SwftCd VARCHAR(30), SvcLvl VARCHAR(  
  30), LclInstrm VARCHAR(30), CtgyPurp VARCHAR(30), OrgnTrxID Varchar(70)  
 )  
...
WHERE t_TrxClearing.TrxBatchID = #TempOutClearing.TrxBatchID  
 ...
 AND t_TrxClearing.TrxDATE BETWEEN @FromDate  
  AND @ReadDate  
...
WHERE t_AccountTrxClearing.TrxBatchID = #TempOutClearing.TrxBatchID  
 ...
 AND t_AccountTrxClearing.TrxDate BETWEEN @FromDate  
  AND @ReadDate  
...
UPDATE t_TrxClearing   
SET ReturnCodeID = t_IncomingTransactions.ReturnCode  
FROM t_IncomingTransactions  
WHERE t_TrxClearing.ColumnID = t_IncomingTransactions.ColumnID  
AND t_TrxClearing.TrxType = 'ID'  
...
BEGIN  
 ...
 SELECT RIGHT(OurBranchID, 4), dbo.f_GetAccountProductID(OurBranchID, AccountID) ProductID, AccountID, TrxDate, ValueDate, ImageID, AccountTypeID   
  AccountType, TrxType, VoucherCode, ReturnCodeID ReturnCode, ABS(Amount), BankID, BranchID, dbo.fn_ExtractChequePrefexed(dbo.fn_GetClearingInwardsTrxID(ColumnID)) ChequeId,  
  ChequeDigit, CASE   
 ...
 UNION ALL  
 SELECT RIGHT(OurBranchID, 4), dbo.f_GetAccountProductID(OurBranchID, AccountID) ProductID, AccountID, TrxDate, ValueDate, ImageID, AccountTypeID   
  AccountType, TrxType, VoucherCode, ReturnCodeID ReturnCode, ABS(Amount), BankID, BranchID,  IsNUll(dbo.fn_ExtractChequePrefexed(dbo.fn_GetClearingInwardsTrxID(ColumnID)),ChequePrefixed) ChequeId, ChequeDigit, CASE   
   WHEN ReturnCodeID = '00'  
    THEN DrawerOrPayeeAccountID  
   ELSE DrawerOrPayeeAccountID  
   END TheirAccount, '', '', '', '', DrawerOrPayee, '', '', 0, 0, '', '', '', '', GETDATE(), ImageID OutwardSerialID, TrxRowID ColumnID, IsNull(  
 ...
 UNION ALL  
 SELECT RIGHT(OurBranchID, 4), dbo.f_GetAccountProductID(OurBranchID, AccountID) ProductID, AccountID, TrxDate, ValueDate, ImageID, AccountTypeID   
  AccountType, TrxType, VoucherCode, ReturnCodeID ReturnCode, ABS(Amount), BankID, BranchID, IsNull(DRN,ChequePrefixed) ChequeId, ChequeDigit, CASE   
   WHEN ReturnCodeID = '00'  
    THEN DrawerOrPayeeAccountID  
   ELSE DrawerOrPayeeAccountID  
   END TheirAccount, '', '', '', '', DrawerOrPayee, '', '', 0, 0, '', '', '', '', GETDATE(), ImageID OutwardSerialID, TrxRowID ColumnID, IsNull(  
 ...
 WHERE TrxDATE BETWEEN @FromDate  
   AND @ReadDate  
  AND TrxType = 'OC'  
  AND IsNull(IsGenerated, 0) = 0  
  AND isNull(IsDeleted, 0) = 0  
...
SELECT *  
INTO #ChequeIMages  
FROM dbo.syn_t_ChequeImages(NOLOCK)  
...
BEGIN  
 UPDATE #MICRCheque  
 SET TrxID = t_IncomingTransactionsHistory.TrxID  
 FROM t_IncomingTransactionsHistory  
 WHERE OriginalColumnID = t_IncomingTransactionsHistory.ColumnID  
  AND #MICRCheque.ReturnCode <> '00'  
...
WHERE #MICRCheque.TrxID = t_IncomingClrTrxExtraDetails.TrxID  
 AND #MICRCheque.ReturnCode <> '00'  
...
WHERE a.CurrencyID = b.CurrencyCode  
...
WHERE a.AccountID LIKE '%' + b.AccountID + '%'  
...
UPDATE #MICRCheque  
SET BankID = Right('00' + rtrim(BankID), 2), BranchID = Right('0000' + rtrim(BranchID), 4)  
...
WHERE a.TransactionID = B.ImageID  
...
BEGIN TRY  
 ...
 SET OrgnlInstrID = b.OrgnlInstrID, OrgnlEndToEnd = b.OrgnlEndToEnd, OrgnlMsgId = b.OrgnlMsgId, MndtId = b.MndtId, DtOfSgntr = b.DtOfSgntr,   
  ReqdColltnDt = isNUll(TRY_CONVERT(DATETIME, TRY_CONVERT(VARCHAR(10),   
  dbo.ChangeDateFormatFromUK(b.ReqdColltnDate), 101), 101), CONVERT  
   (VARCHAR, CONVERT(DATETIME, b.ReqdColltnDate), 106)),  
  Frqcy = b.Frqcy, UstrdColD = b.UstrdColD, UstrdChqdt = b.UstrdChqdt, UstrdBWF = b.UstrdBWF, UstrdBWR = b.UstrdBWR, UstrdGS = b.UstrdGS, UstrdUV = b  
  .UstrdUV, UstrdMicr = b.UstrdMicr, DAdrLine = b.DAdrLine, DTwnNm = b.DTwnNm, DCtry = b.DCtry, DNm = b.DNm, DCNm = b.DCNm, DPhneNb = b.DPhneNb, DMobNb   
  = b.DMobNb, DEmailAdr = b.DEmailAdr, DOthr = b.DOthr, CAdrLine = b.CAdrLine, CTwnNm = b.CTwnNm, CCtry = b.CCtry, CNm = b.CNm, CCNm = b.CCNm, CPhneNb = b  
  .CPhneNb, CMobNb = b.CMobNb, CEmailAdr = b.CEmailAdr, COthr = b.COthr, DbtrAcct = b.DbtrAcct, CdtrAcct = b.CdtrAcct, SvcLvl = b.SvcLvl, LclInstrm =   
  b.LclInstrm, CtgyPurp = b.CtgyPurp  
 FROM #MICRCheque a, t_IncomingClrTrxExtraDetails b  
 WHERE a.TrxID = B.TrxID  
  AND a.ReturnCode <> '00'   
  AND VoucherCode NOT IN (  
   '58'  
   ,'59'  
...
BEGIN CATCH  
 UPDATE #MICRCheque  
 SET OrgnlInstrID = b.OrgnlInstrID, OrgnlEndToEnd = b.OrgnlEndToEnd, OrgnlMsgId = b.OrgnlMsgId, MndtId = b.MndtId, DtOfSgntr = b.DtOfSgntr,   
  ReqdColltnDt = CONVERT(VARCHAR, CONVERT(DATETIME, b.ReqdColltnDate), 106),  
  Frqcy = b.Frqcy, UstrdColD = b.UstrdColD, UstrdChqdt = b.UstrdChqdt, UstrdBWF = b.UstrdBWF, UstrdBWR = b.UstrdBWR, UstrdGS = b.UstrdGS, UstrdUV = b  
  .UstrdUV, UstrdMicr = b.UstrdMicr, DAdrLine = b.DAdrLine, DTwnNm = b.DTwnNm, DCtry = b.DCtry, DNm = b.DNm, DCNm = b.DCNm, DPhneNb = b.DPhneNb, DMobNb   
  = b.DMobNb, DEmailAdr = b.DEmailAdr, DOthr = b.DOthr, CAdrLine = b.CAdrLine, CTwnNm = b.CTwnNm, CCtry = b.CCtry, CNm = b.CNm, CCNm = b.CCNm, CPhneNb = b  
  .CPhneNb, CMobNb = b.CMobNb, CEmailAdr = b.CEmailAdr, COthr = b.COthr, DbtrAcct = b.DbtrAcct, CdtrAcct = b.CdtrAcct, SvcLvl = b.SvcLvl, LclInstrm =   
...
BEGIN  
 INSERT INTO #MICREFTs (  
  OurBranchID, ProductID, AccountID, DATE, ValueDate, AccountType,   
  TrxType, VoucherCode, ReturnCode, Amount, BankID, BranchID, TheirAccountID,  
  ...
  OrgnlEndToEnd, MndtId, DtOfSgntr, ReqdColltnDt, Frqcy, UstrdColD, UstrdChqdt, UstrdBWF, UstrdBWR, UstrdGS, UstrdUV, UstrdMicr, DAdrLine, DTwnNm,  
  DCtry, DNm, DPhneNb, DMobNb, DEmailAdr, DOthr, CAdrLine, CTwnNm, CCtry, CNm, CPhneNb, CMobNb, CEmailAdr, COthr, DCNm, CCNm, CdtrAcct,   
  DbtrAcct, OrgnlMsgId, SwftCd, SvcLvl, LclInstrm, CtgyPurp, OrgnTrxID  
  )  
 SELECT RIGHT(OurBranchID, 4), dbo.f_GetAccountProductID(OurBranchID, AccountID) ProductID, AccountID, TrxDATE, TrxDATE, AccountTypeID AccountType,   
  TrxType, VoucherCode, ReturnCodeID ReturnCode, Abs(Amount), BankID, BranchID, DrawerOrPayeeAccountID TheirAccount,  
  '', '', '', '',DrawerORPayee  ExtraDetails, dbo.f_GetAccountName(OurBranchID, AccountID) Name,  
  -- our customer's name                 
  0, 0, OriginatorCode, OrigRefCode OriginatorReference, PolicyNumber1, PolicyNumber2, DrawerORPayee Remarks, 1, GETDATE(),   
  TrxRowID ColumnID, IsNull(IsGenerated, 0), isNull(Reference, TrxRowID), TrxRowID OutwardSerialID, TrxRowID ColumnID, ColumnID, DRN, '',  ColumnID,   
  '', '', '', '', '', '', '', '', '', '', '', '', '', '',   
  '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '',  
  '', '', '','', '', '',''  
 FROM t_TrxClearing(NOLOCK)  
...
SET SwftCd = SourceBIC  
FROM t_IncomingClrTrxExtraDetails  
WHERE #MICREFTs.TrxID = t_IncomingClrTrxExtraDetails.TrxID  
 AND #MICREFTs.ReturnCode <> '00'  
...
WHERE a.MndtId = b.DDID  
 AND isNull(a.MndtId, '') = ''  
 AND a.VoucherCode = '40'  
  
UPDATE #MICREFTs  
SET MndtId = b.TrxRowID  
FROM #MICREFTs a, t_trxClearing b  
...
UPDATE #MICREFTs  
SET TrxID = b.TrxID  
FROM #MICREFTs a, t_IncomingTransactionsHistory b  
WHERE a.OriginalColumnID = b.ColumnID  
 AND isNull(a.TrxID, '') = ''  
...
BEGIN TRY  
 ...
 SET OrgnlInstrID = b.OrgnlInstrID, OrgnlEndToEnd = b.OrgnlEndToEnd, MndtId = b.MndtId, DtOfSgntr = b.DtOfSgntr, OrgnlMsgId = b.OrgnlMsgId,   
  ReqdColltnDt = isNULL(CONVERT(DATETIME, CONVERT(VARCHAR(10), dbo.ChangeDateFormatFromUK(b.ReqdColltnDate), 101), 101), CONVERT(VARCHAR  
    , CONVERT(DATETIME, b.ReqdColltnDate), 106)),  
  Frqcy = b.Frqcy, UstrdColD = b.UstrdColD, UstrdChqdt = b.UstrdChqdt, UstrdBWF = b.UstrdBWF, UstrdBWR = b.UstrdBWR, UstrdGS = b.UstrdGS, UstrdUV = b  
  .UstrdUV, UstrdMicr = b.UstrdMicr, DAdrLine = b.DAdrLine, DTwnNm = b.DTwnNm, DCtry = b.DCtry, DNm = b.DNm, DCNm = b.DCNm, DPhneNb = b.DPhneNb, DMobNb   
  = b.DMobNb, DEmailAdr = b.DEmailAdr, DOthr = b.DOthr, CAdrLine = b.CAdrLine, CTwnNm = b.CTwnNm, CCtry = b.CCtry, CNm = b.CNm, CCNm = b.CCNm, CPhneNb = b  
  .CPhneNb, CMobNb = b.CMobNb, CEmailAdr = b.CEmailAdr, COthr = b.COthr, DbtrAcct = b.DbtrAcct, CdtrAcct = b.CdtrAcct, SvcLvl = b.SvcLvl, LclInstrm =   
  b.LclInstrm, CtgyPurp = b.CtgyPurp, OrgnTrxID = b.TrxID  
 FROM #MICREFTs a, t_IncomingClrTrxExtraDetails b  
 WHERE a.TrxID = B.TrxID  
  AND a.ReturnCode <> '00'   
  AND VoucherCode IN ('40')  
END TRY  
...
FROM #MICREFTs a, t_TrxClearing b(NOLOCK)  
WHERE a.TransactionMicrColumnID = b.TrxRowID  
 AND a.ReturnCode = '00'  
 AND a.VoucherCode = '40'  
 AND IsNull(a.CNm, '') = ''  
...
--Cancelled EFTs  
UPDATE #MICREFTs  
SET OrgnlInstrID = b.TrxRowID, OrgnlEndToEnd = b.TrxRowID, OrgnlMsgId = b.Reference, ReqdColltnDt = b.TrxDATE, DNm = dbo.f_GetAccountName(b.OurBranchID, b  
