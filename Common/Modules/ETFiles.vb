...
Imports ct812 = BrClearing.Common.BRISO20022CT812
...
#Region "Structures"
    ...
    End Structure
    Public Structure ETACHResponse
        Dim RtrId As String
        Dim OrgnlMsgId As String
        Dim OrgnTrxID As String
        Dim OrgnlInstrID As String
...
#Region "Methods"
        Public Shared Function GenerateETFiles(ByVal FileType As FileType, ByVal CurrCode As Int32, ByVal Exclude As Boolean, Optional ByVal chqFormat As ChequeFormat = ChequeFormat.XMLPackages, Optional ByVal Session As Int16 = 1, Optional ByVal x As String = "", Optional ByVal y As String = "") As Boolean
            Try
                ...
                Try
                    ...
                    Select Case FileType
                        Case FileType.Cheques, FileType.ChequeReturn
                            Modscan.ExecuteData(Modscan.GetModify("Proc_CreateXMLFiles_ET_CTS", "OurBankID", Modscan.OurBankID, "ourbranchid", Modscan.OurBranchID, "ReadDate", Modscan.cWORKING_DATE, "FromDate", Modscan.cFromDate, "EJDate", Modscan.cToDate, "ClearingCenters", "47", "AllCenters", 0, "Currency", CurrCode, "FileType", "J", "Sessno", Session), Modscan.publicDTbl, Modscan.dataExecTypes.ExecTypeQuery, Modscan.queryType.SelectStatement, Modscan.publicDset)
                        ...
                        Case FileType.Efts, FileType.EftReturn, FileType.DD, FileType.DDReturn
                            Modscan.ExecuteData(Modscan.GetModify("Proc_CreateXMLFiles_ET_CTS", "OurBankID", Modscan.OurBankID, "ourbranchid", Modscan.OurBranchID, "ReadDate", Modscan.cWORKING_DATE, "FromDate", Modscan.cFromDate, "EJDate", Modscan.cToDate, "ClearingCenters", "47", "AllCenters", 0, "Currency", CurrCode, "FileType", "T", "Sessno", Session), Modscan.publicDTbl, Modscan.dataExecTypes.ExecTypeQuery, Modscan.queryType.StoredProcedure, Modscan.publicDset)
                            If Modscan.publicDTbl.Rows.Count > 0 Then
                                publicDTblBankCopy = Modscan.publicDset.Tables(0).Clone()
                                distinctBankID = Modscan.publicDset.Tables(0).Clone()
                    ...
                    Case FileType.Cheques
                        For k As Int32 = 0 To BankArr.Count - 1
                            Try
                                ...
                                Dim BIC As String = ""
                                'Ejs
                                Modscan.ExecuteData(Modscan.GetModify("Proc_CreateXMLFiles_ET_CTS", "OurBankID", Modscan.OurBankID, "ourbranchid", Modscan.OurBranchID, "ReadDate", Modscan.cWORKING_DATE, "FromDate", Modscan.cFromDate, "EJDate", Modscan.cToDate, "ClearingCenters", "47", "AllCenters", 0, "Currency", CurrCode, "FileType", "J", "Sessno", Session), Modscan.publicDTbl, Modscan.dataExecTypes.ExecTypeQuery, Modscan.queryType.StoredProcedure)
                    ...
                    Case FileType.Efts
                        ...
                        For k As Int32 = 0 To BankArr.Count - 1
                            Try
                                Dim i As Integer = 0
                                Dim amt As Decimal = 0
                                Dim l As New List(Of ET.ETChequeDetails)
                                Dim FileCounter As String = GetScalarREC("Select isNull(FileCounter,0)+1 From t_Bank Where BankID = '" & BankArr.Item(k) & "'")
                                Dim CreateFile As String = Session & Now.ToString(".yyyyMMdd.HHmm.") & BankArr.Item(k) & "." & Session
                                Dim BIC As String = ""
                                'EFT Cr
                ...
                End Select
                GenerateETFiles = True
                publicDTblBankCopy.Clear()
            Catch ex As Exception
                GenerateETFiles = False
            End Try
        End Function
        Private Shared Function GetScalarREC(ByVal strStatementORstrProcedure As String, Optional ByVal strArr As String = "") As String
            ...
            Return strResults
        End Function
        Private Shared Function BulkCredit(ByVal l As List(Of ETEFTDetails), ByVal ccy As String, ByVal BIC As String, ByVal x As String, ByVal y As String) As String
            ...
            Dim doc As New ct812.Document()
            Dim grpHdr As New ct812.GroupHeader33
            grpHdr.MsgId = "OD/" & Modscan.WORKING_DATE.ToString("ddMMyy") & "/" & Modscan.Sess & TimeSec
            grpHdr.CreDtTm = xDt
            grpHdr.NbOfTxs = l.Count
            grpHdr.TtlIntrBkSttlmAmt = New ct812.ActiveCurrencyAndAmount() With {.Ccy = stCurrCode, .Value = Decimal.Round(CDec(dAmt), 2)}
            grpHdr.IntrBkStDate = sDt
            grpHdr.PmtTpInf = New ct812.PaymentTypeInformation21() With {.LclInstrm = New ct812.LocalInstrument2Choice() With {.ItemElementName = ct812.ItemChoiceType4.Cd, .Item = "08"}}
            grpHdr.SttlmInf = New ct812.SettlementInformation13() _
             With {.SttlmMtd = ct812.SettlementMethod1Code.CLRG, .ClrSys = New ct812.ClearingSystemIdentification3Choice() _
                  With {.Item = "47"}}
            grpHdr.InstgAgt = New ct812.BranchAndFinancialInstitutionIdentification4 With {.FinInstnId = New ct812.FinancialInstitutionIdentification7 With {.BIC = BIC.ToUpper, .ClrSysMmbId = New ct812.ClearingSystemMemberIdentification2() With {.MmbId = l(0).SourceBankID}}}
            grpHdr.InstdAgt = New ct812.BranchAndFinancialInstitutionIdentification4 With {.FinInstnId = New ct812.FinancialInstitutionIdentification7 With {.BIC = l(0).DestBIC.ToUpper, .ClrSysMmbId = New ct812.ClearingSystemMemberIdentification2() With {.MmbId = l(0).DestBankID}}}
            ...
            For Each itm As ETEFTDetails In l
                ...
                Dim cdtTxn As New ct812.CreditTransferTransactionInformation11()
                cdtTxn.PmtId.EndToEndId = "EFT/" & itm.TrxId
                cdtTxn.PmtId.InstrId = itm.VCode
                cdtTxn.PmtId.TxId = itm.TrxId
                cdtTxn.PmtTpInf = New ct812.PaymentTypeInformation21() _
                   With {.CtgyPurp = New ct812.CategoryPurpose1Choice() With {.Item = itm.VCode}}
                cdtTxn.IntrBkSttlmAmt = New ct812.ActiveCurrencyAndAmount() With {.Ccy = stCurrCode, .Value = Decimal.Round(CDec(amt), 2)}
                'cdtTxn.IntrBkSttlmDt = xDt
                cdtTxn.ChrgBr = ct812.ChargeBearerType1Code.SHAR
                cdtTxn.Dbtr = New ct812.PartyIdentification32() _
                With {.Nm = itm.DNm.ToUpper, .PstlAdr = New ct812.PostalAddress6 With {.TwnNm = .TwnNm, .Ctry = .Ctry, .AdrLine = {""}}}
                cdtTxn.DbtrAcct = New ct812.CashAccount16() With {.Id = New ct812.AccountIdentification4Choice() With {.Item = New ct812.GenericAccountIdentification1() With {.Id = itm.DbtrAcct}}}
                cdtTxn.DbtrAgt = New ct812.BranchAndFinancialInstitutionIdentification4 With {.FinInstnId = New ct812.FinancialInstitutionIdentification7() With {.BIC = itm.OurBankBic.ToUpper, .ClrSysMmbId = New ct812.ClearingSystemMemberIdentification2() With {.MmbId = itm.SourceBankID}}}
                cdtTxn.Cdtr = New ct812.PartyIdentification32() With {.Nm = itm.CNm.ToUpper}
                cdtTxn.CdtrAcct = New ct812.CashAccount16() With {.Id = New ct812.AccountIdentification4Choice() With {.Item = New ct812.GenericAccountIdentification1() With {.Id = itm.CdtrAcct}}}
                cdtTxn.CdtrAgt = New ct812.BranchAndFinancialInstitutionIdentification4 With {.FinInstnId = New ct812.FinancialInstitutionIdentification7() With {.BIC = itm.DestBIC.ToUpper, .ClrSysMmbId = New ct812.ClearingSystemMemberIdentification2() With {.MmbId = itm.DestBankID}}}
                cdtTxn.RmtInf = New ct812.RemittanceInformation5() With {.Ustrd = {itm.UstrdColD.ToUpper}}
        ...
        Private Shared Function SignFiles_PKCS(ByVal Sourcepath As String, ByVal DestPath As String, ByVal cert As String, ByVal tokenpass As String) As String
            ...
            Return Mes
        End Function
        Private Shared Function CancelCredit(ByVal l As List(Of ETEFTDetails), ByVal ccy As String, ByVal BIC As String, ByVal x As String, ByVal y As String) As String
            Dim RegX As New Regex("[^A-Za-z0-9]")
            Dim dAmt As Decimal = 0
            For Each itm As ETEFTDetails In l
                dAmt += CDec(itm.Amount)
            ...
            For Each d As ETEFTDetails In l
                Try
        ...
        Private Shared Function BulkDebit(ByVal l As List(Of ETDDDetail), ByVal ccy As String, ByVal BIC As String, ByVal x As String, ByVal y As String) As String
            Try
        ...
        Private Shared Function DDReject(ByVal l As List(Of ETDDDetail), ByVal ccy As String, ByVal BIC As String, ByVal x As String, ByVal y As String) As String
            Dim RegX As New Regex("[^A-Za-z0-9]")
            Dim dAmt As Decimal = 0
            For Each itm As ETDDDetail In l
                dAmt += CDec(itm.Amount)
        ...
        Private Shared Function BulkCheques(ByVal det As List(Of ETChequeDetails), ByVal ccy As String, ByVal amt As Decimal, ByVal BIC As String, ByVal x As String, ByVal y As String) As String
            Dim sDt As String = Modscan.FDATE.ToString("dd-MMM-yyyy")
            Dim STm As String = Now.ToString("HH:mm")
            Dim TimeSec As String = Now.ToString("HHmmss")
        ...
        Private Shared Function UnpaidCheques(ByVal l As List(Of ETChequeDetails), ByVal BIC As String, ByVal ccy As String, ByVal x As String, ByVal y As String) As String
            Dim RegX As New Regex("[^A-Za-z0-9]")
            Dim dAmt As Decimal = 0
            For Each itm As ETChequeDetails In l
                dAmt += CDec(itm.Amount)
            Next
            Dim amt As String = FormatNumber(dAmt, 2, Microsoft.VisualBasic.TriState.True, Microsoft.VisualBasic.TriState.True, Microsoft.VisualBasic.TriState.False)
            Dim sDt As String = Modscan.FDATE.ToString("dd-MMM-yyyy")
...
#Region "Methods"
                ...
                For Each sFile As String In sFiles
                    Try
                        ...
                        If siExt.Contains("ZIP") Then
                            FExt = ".ZIP"
                        End If
                        If siExt.Contains("S") Then
                            FExt = ".S"
                        End If
                        If (siExt.Contains("N")) Then
                            FExt = ".N"
                        End If
                        ...
                        Select Case FExt.ToUpper
                            Case ".ZIP"
                                ...
                                Else
                                    ...
                                    End If
                                    Dim l As List(Of String) = UnzipFiles(sFile, New String() {"*.xml*", "*.tiff"})
                                    If l.AsQueryable().Any(Function(p) p.Contains("xml") Or p.Contains("tiff")) Then
                                        Try
        ...
        Private Shared Sub BulkCredit(ByVal sFile As String)
            ...
            For f As Int32 = 0 To ReWorkedStringCol.Count - 1
                ...
                Dim doc As New ct812.Document()
                Dim ex As New Exception()
                If ct812.Document.LoadFromFile(sFile, doc, ex) Then
                    For Each c As ct812.CreditTransferTransactionInformation11 In doc.FIToFICstmrCdtTrf.CdtTrfTxInf
                        Dim d As New ETEFTDetails
        ...
        '    Dim chq As New ET.ChequeDetails
        ...
        Private Shared Function RejectedItems(ByVal sFile As String) As Boolean
            ...
            Return IsReject
        End Function
        Private Shared Sub UnpayItem(ByRef ACHRes As ETACHResponse)
        ...
        End Sub
        Private Shared Sub UnpayCheque(ByRef ch As ET.ETChequeDetails)
            Try
        ...
        Private Sub UnpayEft(ByRef d As ET.ETEFTDetails)
            'Try
            '    Dim dtUnpaid As New DataTable()
