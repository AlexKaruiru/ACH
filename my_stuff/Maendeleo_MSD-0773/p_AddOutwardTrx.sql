...
BEGIN  
 ...
 IF EXISTS (  
   ...
   WHERE ChequeID = @ChequeID  
    AND BankID = @BankID  
    AND DrawerOrPayeeAccountID = @DrawerOrPayeeAccountID  
    AND BranchID = @BranchID  
    AND TrxType = 'OC'  
    AND isNull(IsDeleted, 0) = 0  
    AND Abs(Amount) = Abs(@Amount)  
   )  
 BEGIN  
  RAISERROR (  
    N'BREXDB999955'  
 ...
 IF @AccountTypeID = 'G'  
  AND @AccountID NOT IN (  
   dbo.f_GetCurrencyBranchGLAccountID(@OurBranchID, @TrxCurrencyID, 'ACP_CLR_SUSP_AC_EFT')
   ,dbo.f_GetCurrencyBranchGLAccountID(@OurBranchID, @TrxCurrencyID, 'CEN_BANK_AC')
 ...
 BEGIN  
  IF EXISTS (  
    ...
    WHERE ColumnID = TRY_PARSE(@ReferenceNo AS BIGINT)  
    )  
  BEGIN  
 ...
 IF (  
   ...
   FROM t_AccountSpecialCondition(NOLOCK)  
   WHERE OurBranchID = @OurBranchID  
    AND AccountID = @AccountID  
    AND SpecialConditionID = '108'  
   ) <> 0  
 BEGIN  
 ...
 IF @TrxTypeID = 'OC'  
  AND @ReturnCodeID IN ('00')  
  AND @OldVATSerialID NOT IN (  
   '7777MFI'  
   ,'7778MFI'  
   )  
 BEGIN  
 ...
 IF @ReturnCodeID NOT IN (  
   '00'  
   ,'17'  
   )  
  AND @TrxTypeID = 'OC'  
 BEGIN  
  SET @TrxDescriptionID = CASE   
    WHEN @ReturnCodeID IN ('63')  
     THEN '086'  
    WHEN @ReturnCodeID = '62'  
     THEN '087'  
    WHEN @ReturnCodeID IN (  
      '33'  
      ,'37'  
      ,'38'  
      ,'55'  
 ...
 --- Valuecaping          
 SELECT @ValueCapingAmt = ISNULL(ValueCappingAmount, 999999)  
 FROM t_Currency(NOLOCK)  
 WHERE CurrencyID = @TrxCurrencyID  
 ...
 IF @TrxTypeID = 'OD'  
 BEGIN  
  SELECT @ContraGLID = dbo.f_GetCurrencyBranchGLAccountID(@OurBranchID, @TrxCurrencyID, 'CUR_CLR_AC_IN') --_EFT')    
 END  
 ELSE IF @TrxTypeID = 'OC' AND REPLACE(@ReturnCodeID,'-','') IN ('00','17')  
 BEGIN   
 ...
 SET @HOBranchLocalBranchCurrency = (  
   SELECT CurrencyID  
 ...
 BEGIN  
  ...
  IF ISNULL(@HOTrxDate, @TrxDate) <> @TrxDate  
   OR @DestinationTrxDate <> @TrxDate  
  BEGIN  
   IF (  
     SELECT dbo.f_SystemBankParameterExists(@OurBrankID, 34)  
     ) <> 1  
   BEGIN  
    RAISERROR (  
      'BREXDB301003'  
  ...
  BEGIN  
   ...
   IF (@HOTrxDate <> @TrxDate)  
    OR (@DestinationTrxDate <> @TrxDate)  
    OR (@HOTrxDate <> @DestinationTrxDate)  
   BEGIN  
    RAISERROR (  
      'BREXDB301003'  
      ,16  
   ...
   IF ISNULL(@SourceIBGLID, '') = ''  
    OR ISNULL(@HOIBCreditGLID, '') = ''  
    OR ISNULL(@HOIBDebitGLID, '') = ''  
    OR ISNULL(@DestinationIBGLID, '') = ''  
   BEGIN  
    RAISERROR (  
      N'BREXDB817051'  
      ,16  
 ...
 BEGIN  
  ...
  IF EXISTS (  
    SELECT DISTINCT t_Transaction.TrxRowID  
    FROM t_Transaction(NOLOCK)  
    INNER JOIN t_TrxClearing(NOLOCK) ON t_Transaction.TrxBatchID = t_TrxClearing.TrxBatchID  
 ...
 BEGIN  
  ...
  BEGIN  
   SELECT @OrigCode = OriginatorCode  
    ,@OrigRef = OrigRefCode  
    ,@Policy1 = PolicyNumber1  
    ,@Policy2 = PolicyNumber2  
    ,@ColumnID = ColumnID  
   FROM t_TrxClearing NOLOCK  
   WHERE ImageID = @ImageID  
    AND TrxType = 'ID'  
 ...
 SELECT ISNULL(@NewRowID, 0)  
  ...
  ,CASE @VoucherCode  
   ...
   WHEN '39'  
    THEN '00'  
   ELSE dbo.f_CRB_ReturnCodeDescriptionsForUnpay(@ReturnCodeID)  
   END  
  ,@Commission  
  ,@TheirCommission  
  ,@BankID  
 ...
 BEGIN  
  ...
  SET ReturnCodeID = a.ReturnedTempID  
  FROM (  
   SELECT ColumnID  
    ,ReturnedTempID  
   FROM t_IncomingTransactions  
   WHERE ColumnID IN (  
     SELECT ColumnID  
     FROM t_TrxClearing  
     WHERE isNull(ColumnID, '') <> ''  
      AND TrxType IN (  
       'OC'  
       ,'OD'  
       )  
     )  
    AND isNull(ReturnedTempID, '') <> ''  
   ) AS A  
  WHERE t_TrxClearing.ColumnID = a.ColumnID  
   AND t_TrxClearing.TrxType IN (  
    'OC'  
    ,'OD'  
    )  
   AND isNull(ReturnCodeID, '') IN (  
    '00'  
    ,'17'  
    )  
  ...
  END  
    --SET @TrxDescriptionID = CASE WHEN @VoucherCode IN ('39','40') THEN '089' ELSE @TrxDescriptionID END    
 END  
 ELSE IF @ReturnCodeID IN ('00')  
  AND @TrxTypeID IN ('OC')  
  AND @OldVATSerialID = '7777MFI'  
 BEGIN TRY  
 ...
 UPDATE t_TrxClearing  
 SET TrxRowID = t_Transaction.TrxRowID  
 FROM t_Transaction(NOLOCK)  
 WHERE t_Transaction.TrxBatchID = t_TrxClearing.TrxBatchID  
  AND t_Transaction.OurBranchID = t_TrxClearing.OurBranchID  
 ...
 BEGIN  
  UPDATE t_TrxClearing  
  SET BankID = t_TrxInwards.BankID  
   ,BranchID = t_TrxInwards.BranchID  
  FROM t_TrxInwards(NOLOCK)  
  WHERE t_TrxInwards.ColumnID = t_TrxClearing.ColumnID  
   AND t_TrxClearing.TrxType IN (  
    'ID'  
    ,'IC'  
    )  
...
SET @TrxDescription = CASE   
  WHEN @TrxTypeID IN (  
    'OC'  
    ,'TC'  
    )  
   THEN @TrxDescription + ' : ' + cast(@ChequeID AS VARCHAR)  
  ELSE @TrxDescription  
  END  
...
IF @TrxDescriptionID = '086'  
 AND @TrxCurrencyID = 'USD'  
 AND @TrxTypeID IN (  
  'OC'  
  ,'TC'  
  )  
...
END  
ELSE IF @TrxDescriptionID = '086'  
 AND @TrxCurrencyID = 'GBP'  
 AND @TrxTypeID IN (  
  'OC'  
  ,'TC'  
  )  
