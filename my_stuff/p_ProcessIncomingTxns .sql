...
--RETURN  
 ...
 BEGIN  
  IF EXISTS (SELECT 1 FROM syn_t_IncomingTransactions WHERE ColumnID = @ColumnID AND isNull(Verified,0) = 0)  
   BEGIN  
    UPDATE syn_t_IncomingTransactions   
 ...
 BEGIN  
  UPDATE syn_t_IncomingTransactions SET ReturnedTempID = @ReturnCodeID WHERE ColumnID = @ColumnID  
 END  
 ELSE IF isNull(@ReturnCodeID,'') = ''  
 BEGIN  
  UPDATE syn_t_IncomingTransactions SET ReturnedTempID = '00' WHERE ColumnID = @ColumnID  
 END  
 ...
 FROM syn_t_IncomingTransactions   
 WHERE ColumnID = @ColumnID  
...
--RETURN  
 ...
 IF @AccountID <> dbo.f_GetCurrencyBranchGLAccountID(@TrxBranchID, ISNULL(@TrxCurrencyID, 'TZS'), 'ACP_CLR_SUSP_AC') AND @OriginalReturnCode IN ('00','17') AND @TrxTypeID = 'ID' AND @ReturnedTempID<>'00'  
 BEGIN  
...
--RETURN  
 ...
 IF isNull(@Verified,0) = 0  
  BEGIN  
   ...
   SELECT * FROM syn_t_IncomingTransactions WHERE COLUMNID=62  
--SELECT @TrxTypeID,@TxnType  
--RETURN  
  END  
...
--RETURN  
   ...
   UPDATE syn_t_IncomingTransactions   
   SET TrxBranchID = @OurBranchID  
   WHERE ColumnID = @TrxRowID AND AccountID IN (dbo.f_GetCurrencyBranchGLAccountID(@TrxBranchID, @TrxCurrencyID, 'ACP_CLR_SUSP_AC'),  
   dbo.f_GetCurrencyBranchGLAccountID(@TrxBranchID, @TrxCurrencyID, 'ACP_CLR_SUSP_AC'))  
-- SELECT @TrxTypeID  
--RETURN  
     ...
     BEGIN  
      ...
      EXEC p_AddOutwardTrx    
        ...
        BEGIN  
         ...
         BEGIN TRY    
         --  
          INSERT INTO dbo.syn_t_ChequeImages         
          (      
          ...
          FROM syn_t_IncomingTransactions  NOLOCK        
          WHERE ColumnID = @ColumnID   
         END TRY    
         BEGIN CATCH   
 ...
 ELSE IF @TxnType='UnPay'  
  BEGIN  
   ...
   -- UPDATE syn_t_IncomingTransactions SET BankID = RIGHT(@FileName,2) WHERE ColumnID = @ColumnID  
   --END  
   --print  
...
--RETURN   
   ...
   BEGIN TRAN POSTUNPAY  
    ...
    IF EXISTS (SELECT 1 FROM syn_t_IncomingTransactions WHERE ColumnID = @TrxRowID AND isNull(Verified,0) = 0)  
     BEGIN  
      UPDATE syn_t_IncomingTransactions   
      SET  Returned = 1, Paid = 1, isProcessed = 1, Verified = 1, VerifiedBy = @OperatorID,   
        VerifiedOn = @WorkingDate, ClearedDate = @WorkingDate, TrxBatchID = @TrxBatchID,  
  ...
  BEGIN  
   ...
   BEGIN  
    ...
    BEGIN  
     ...
     BEGIN  
      --SELECT @TrxBranchID = @OurBranchID  
      SELECT @AccountID = dbo.f_GetCurrencyBranchGLAccountID(@TrxBranchID, ISNULL(@TrxCurrencyID, 'TZS'), 'ACP_CLR_SUSP_AC')  
      SELECT @OurBranchID = @OurBranchID  
      SELECT @AccountTypeID = 'G'  
      SELECT @ProductID = 'GL'  
      SELECT @MainGLID = dbo.f_GetCurrencyBranchGLAccountID(@TrxBranchID, ISNULL(@TrxCurrencyID, 'TZS'), 'ACP_CLR_SUSP_AC')  
